...
# Mapping: short hash (8 chars) -> new professional message
COMMIT_MESSAGES = {
    ...
    '930acece': 'instrument: add comprehensive pipeline logging for fcc-server/Claude CLI flow debugging',
...
def rewrite_messages(commit):
    ...
    original_hash = commit.original_id[:8].decode('ascii')
    new_msg = COMMIT_MESSAGES.get(original_hash)
    if new_msg:
