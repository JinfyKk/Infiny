...
export function getModelDescription(model: string, provider?: FreeClaudeProviderId): string {
  ...
  const providerNames: Record<FreeClaudeProviderId, string> = {
    ...
    xai: 'via xAI Grok',
    mistral: 'via Mistral',
    mistral_codestral: 'via Mistral Codestral',
    nvidia: 'via NVIDIA NIM',
    bedrock: 'via AWS Bedrock',
    vertex: 'via Vertex AI',
    cloudflare: 'via Cloudflare Workers AI',
    huggingface: 'via Hugging Face',
    github_models: 'via GitHub Models',
    wafer: 'via Wafer',
    kimi: 'via Kimi',
    kimi_code: 'via Kimi Code',
    minimax: 'via MiniMax',
    fireworks: 'via Fireworks',
    sambanova: 'via SambaNova',
    zai: 'via Z.ai',
    opencode: 'via OpenCode Zen',
    opencode_go: 'via OpenCode Go',
    vercel: 'via Vercel AI Gateway',
    ollama_cloud: 'via Ollama Cloud',
    lmstudio: 'via LM Studio (local)',
    llamacpp: 'via llama.cpp (local)',
